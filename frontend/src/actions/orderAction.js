...
// my orders Details
export const myOrdersDetails = (id) => async (dispatch, getState) => {
  try {
    dispatch({ type: MY_ORDERS_DETAILS_REQUEST });

    const { data } = await axios.get(`/api/v1/order/${id}`);

    dispatch({ type: MY_ORDERS_DETAILS_SUCCESS, payload: data.order });
  } catch (error) {
    dispatch({
